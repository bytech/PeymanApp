		  ...
		  $.each(data, function(){
			  ...
			  if(this.CatType){
				  $('#homePage').append(
			  		...
			  		$('#homePage').append(
					  '<li id="post-'+this.ID+'">'
						  + '<a href="'+url+'" data-transition="slide" '+ajax+'>'
								  + '<img class="GroupSaleImage" src="' + this.Image + ' " />'
								  + '<div class="title">'
									  + this.Title
								  + '</div>'
				...
				success: function(data) {
				   ...
				   //--------------------------------------------------------------------------------------
					...
					$.each(data.GroupSales, function(){
						...
						$('#GroupSales').append(
							'<li id="'+ this.GROUPSALEID +'" class="'+ starsImg +'">'
								+ '<a href="'+url+'">'
									+ '<div class="GroupSale">'
										+ '<h2>'
											+ this.HOTELNAME
			  ...
			  success: function(data) {
				 ...
				 //console.log(data.GroupSales[0]);
				  ...
				  $.each(data.Hotels, function(){
					  ...
					  $('#GroupSales').append(
						  '<li id="'+ this.HOTELID +'" class="'+ starsImg +'">'
...
$(document).on('pagebeforeshow', '#hotel_template', function(e, data){
			   ...
			   console.log(data.Hotels.length);*/
				...
				$.each(data.Hotels, function(){
				   ...
				   if(this.PHONE900){
					...
					var imagesSwipe = '<img class="hotel_main_image" src="http://www.hotels.co.il/hotelsmain/hotels/hotelimages/' + this.MAINPIC + ' " />';
					var imagesSwipeNum = 1;
					$.each( temp, function( key, value ){
					  imagesSwipeNum = imagesSwipeNum + 1;	
					  ...
					  imagesSwipe += '<img src="http://www.hotels.co.il/hotelsmain/hotels/hotels_pics/'+value+'" />';
					  //------------------------------------------------------------------------------------------------------
					...
					//console.log(imagesSwipeNum);
					localStorage.setItem("ImagesSwipeNum", imagesSwipeNum);
					localStorage.setItem("HotelGallery", JSON.stringify(HotelGallery));
					//console.log(images);
					...
					$('#two').append(this.SHORTHOTELDESCRIPTION);
					$('#three').append(images);
					$('#imgs').append(imagesSwipe);
	...
	//$('#tow').empty();
	(function($){
		var url = 'http://www.hotels.co.il/json/JPgroupSale.cfc?method=GetGroupSales&HotelID='+HotelID+'';
		  $.ajax({
			type: 'GET',
			url: url,
			async: false,
			...
			success: function(data){
			   ...
			   console.log(data.GroupSales);
				...
				if (data.GroupSales[0].ERROR){
					...
					$('#one').append('<p class="no_deals">דילים חדשים עולים כל יום בשעה 12:00</p>');
				} //if (data.GroupSales[0].ERROR)
				else{
					$.each(data.GroupSales, function(){
							if(this.SALENIGHTS == 1){
